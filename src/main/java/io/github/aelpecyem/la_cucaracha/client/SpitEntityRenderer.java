...
public class SpitEntityRenderer extends EntityRenderer<SpitEntity> {
	private static final Identifier TEXTURE = new Identifier("textures/entity/llama/spit.png");
	private final LlamaSpitEntityModel<SpitEntity> model;

	public SpitEntityRenderer(EntityRendererFactory.Context context) {
		super(context);
		this.model = new LlamaSpitEntityModel<>(context.getPart(EntityModelLayers.LLAMA_SPIT));
	}

	public void render(SpitEntity llamaSpitEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
		matrixStack.push();
	...
	public Identifier getTexture(SpitEntity llamaSpitEntity) {
		return TEXTURE;
	}
}
